...
const ownerData = [['owner@ratespace.demo', 'RateSpace Demo Store Owner Account'], ...Array.from({ length: 11 }, (_, index) => [`owner${index + 2}@ratespace.demo`, `RateSpace Store Owner Account ${index + 2}`])];
async function main() {
    ...
    for (const [email, name, address] of additionalRaters)
        raters.push(await prisma.user.upsert({ where: { email }, update: { name, password: userPassword, role: 'USER', address }, create: { name, email, password: userPassword, role: 'USER', address } }));
    const storeRecords = [];
    ...
    for (const [storeIndex, raterIndex, rating, createdAt] of ratingPlan) {
        const store = storeRecords[storeIndex], rater = raters[raterIndex];
